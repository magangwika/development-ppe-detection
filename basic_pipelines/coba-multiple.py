...
class CustomGStreamerDetectionApp(GStreamerDetectionApp):
    ...
    def get_pipeline_string(self):
        # Get the base pipeline components with QoS disabled
        source_pipeline = (
            f'filesrc location="{self.video_source}" name=source ! '
            f'queue name=source_queue_decode leaky=no max-size-buffers=20 max-size-bytes=0 max-size-time=0 ! '
            f'decodebin name=source_decodebin ! '
            f'queue name=source_scale_q leaky=no max-size-buffers=20 max-size-bytes=0 max-size-time=0 ! '
            f'videoscale name=source_videoscale n-threads=2 qos=false ! '
            f'queue name=source_convert_q leaky=no max-size-buffers=20 max-size-bytes=0 max-size-time=0 ! '
        ...
        tracker_pipeline = TRACKER_PIPELINE(class_id=1, qos=False)
        user_callback_pipeline = USER_CALLBACK_PIPELINE()
        
        # Modified display pipeline with xvimagesink and more buffering
        display_pipeline = (
            f'{OVERLAY_PIPELINE(name="hailo_overlay")} ! '
            #f'queue name=hailo_overlay_q leaky=no max-size-buffers=20 max-size-bytes=0 max-size-time=0 ! '
            f'queue name=hailo_display_videoconvert_q leaky=no max-size-buffers=20 max-size-bytes=0 max-size-time=0 ! '
            f'videoconvert name=hailo_display_videoconvert n-threads=2 qos=false ! '
            f'queue name=hailo_display_q leaky=no max-size-buffers=20 max-size-bytes=0 max-size-time=0 ! '
...
class ImageSender(threading.Thread):
    def __init__(self, mqtt_client):
        ...
        self.image_queue = queue.Queue(maxsize=10)  # Limit queue size
        self.running = True
        self.delay_between_sends = 2.0  # Increased delay
        self.daemon = True
        self.batch_size = 3  # Process multiple images at once
        self.compression_quality = 70  # JPEG compression quality
