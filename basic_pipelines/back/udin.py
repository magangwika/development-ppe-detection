...
class UserAppCallback(app_callback_class):
    ...
    def publish_status_and_frame(self, no_pdl):
        ...
        if self.last_false_frame is not None:
            ...
            _, img_encoded = cv2.imencode('.jpg', self.last_false_frame, [cv2.IMWRITE_JPEG_QUALITY, 80])
...
def setup_mqtt():
   # mqtt_client = mqtt.Client()
    mqtt_client = mqtt.Client(mqtt.CallbackAPIVersion.VERSION2)  # Gunakan versi terbaru
