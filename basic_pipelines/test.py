...
COLORS = {
    ...
    "buruh": (255, 0, 0),       # Biru
    "site-engineer": (0, 0, 255), # Merah
    "unknown": (0, 255, 255)    # Kuning
}
...
class UserAppCallback(app_callback_class):
    ...
    def draw_bounding_box(self, frame, detection, color):
        ...
        # Gambar bounding box dengan sudut yang lebih halus
        thickness = 2
        # Gambar garis horizontal atas
        cv2.line(frame, (x1, y1), (x1 + int(box_width * 0.2), y1), color, thickness)
        cv2.line(frame, (x2 - int(box_width * 0.2), y1), (x2, y1), color, thickness)
        # Gambar garis horizontal bawah
        cv2.line(frame, (x1, y2), (x1 + int(box_width * 0.2), y2), color, thickness)
        cv2.line(frame, (x2 - int(box_width * 0.2), y2), (x2, y2), color, thickness)
        # Gambar garis vertikal kiri
        cv2.line(frame, (x1, y1), (x1, y1 + int(box_height * 0.2)), color, thickness)
        cv2.line(frame, (x1, y2 - int(box_height * 0.2)), (x1, y2), color, thickness)
        # Gambar garis vertikal kanan
        cv2.line(frame, (x2, y1), (x2, y1 + int(box_height * 0.2)), color, thickness)
        cv2.line(frame, (x2, y2 - int(box_height * 0.2)), (x2, y2), color, thickness)
        
        # Gambar label background
        label_text = f"{label}: {confidence:.2f}"
        (text_width, text_height), _ = cv2.getTextSize(label_text, cv2.FONT_HERSHEY_SIMPLEX, 0.5, 1)
        
        # Pastikan label tidak keluar dari frame
        label_x = max(0, min(x1, width - text_width))
        label_y = max(text_height + 4, min(y1, height))
    ...
    def process_detections(self, detections, frame):
        ...
        for detection in detections:
            label = detection.get_label()
            color = COLORS.get(label, (255, 255, 255))  # Default putih jika label tidak dikenal
            self.draw_bounding_box(frame, detection, color)
